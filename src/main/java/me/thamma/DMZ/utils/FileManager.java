...
public class FileManager {
	...
	public static List<FileManager> getFromFolder(String path) {
		...
		List<FileManager> al = new ArrayList<FileManager>();
		if (folder.exists()) {
			...
			for (int i = 0; i < j; i++) {
				...
				if (file.isFile()) {
					al.add(new FileManager((folder.getPath() + "/" + file.getName().replaceFirst("[.][^.]+$", ""))
	...
	private YamlConfiguration loadFile(String path) throws IOException, InvalidConfigurationException {
