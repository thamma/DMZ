...
public class ChunkyCommands implements CommandExecutor {
	static final CommandHandler ch = new CommandHandler("chunky").add(new Argument() {
		...
		public void run(Player p, List<String> args) {
			try {
				...
				msg(p, "&eThe attribute &6" + a.name() + " &efor this chunky is &6" + c.getAttribute(a) + "&e.");
			} catch (Exception e) {
